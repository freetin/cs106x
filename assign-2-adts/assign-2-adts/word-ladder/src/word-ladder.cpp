...
bool norepeat(stack<string>group,string word)//是否重复
{
    while (!group.empty())
    {
        if(group.top()==word)
...
stack<string>change_stack(stack<string>group1)//颠倒栈的顺序
{
    stack<string>group2;
    while (!group1.empty())
    {
...
stack<string>copy_stack(stack<string>group1)//复制栈
{
    stack<string>group2;
    while (!group1.empty())
    {
...
void Finding_word_ladder(string start,string end,const Lexicon& english)
{
    ...
    while(!WordLad.empty())//遍历队列
    {
        for(int i=0;i<WordLad.front().top().length();i++)//队列中栈的top字母
        {
            char a = 'a';
            ...
            for(;a<'z';a++)//字母遍历
            {
                ...
                {
                    word[i]=a;
                    if(english.contains(word))//是否在字典中
                    {
